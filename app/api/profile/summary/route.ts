  ...
  for (const base of recentUsefulWords) {
    ...
    for (let index = 0; index <= base.length; index += 1) {
      for (const letter of ALPHABET) {
        const inserted = `${base.slice(0, index)}${letter}${base.slice(index)}`;
        addRecommendation(candidates, inserted, `one-letter extension of ${base}`, 8, known);
      }
    }
    ...
    for (let index = 0; index < base.length; index += 1) {
      for (const letter of ALPHABET) {
        ...
        const substituted = `${base.slice(0, index)}${letter}${base.slice(index + 1)}`;
        addRecommendation(candidates, substituted, `one-letter swap near ${base}`, 5, known);
      }
    }
