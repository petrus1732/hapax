...
export default function BoardClient({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (board && trie) {
      ...
      const ws = findWords(size, letters, trie)[0].sort((a, b) =>
      ...
      {timeLeft > 0 && board && wordsLength > 0 ? (
        <div>
          ...
          >
            ...
            <Modal.Body
              className="bg-white dark:bg-black overflow-y-auto"
              style={{ maxHeight: 'calc(100vh - 54px - 9rem)' }}
            >
              <div className="m-2">
                <input
                  type="checkbox"
                ...
                />
                <label htmlFor="mask" className="ml-2">
                  reveal letters
                </label>
              </div>
              {isRareLetters ? (
                          ...
                          <ul className="flex flex-wrap">
                            {ws.map((w, idx) =>
                              swiped[w] ? (
                                <li
                                  key={idx}
                                  style={{ width: 272 / ((272 / (id * 15)) | 0) + 'px', color: 'green' }}
                                  className="flex-none cursor-pointer hover:underline"
                                  onClick={() => handleWordClick(w)}
                                >
                                  {w}
                                ...
                                <li
                                  key={idx}
                                  style={{ width: 272 / ((272 / (id * 15)) | 0) + 'px' }}
                                  className="flex-none cursor-pointer hover:underline"
                                  onClick={() => handleWordClick(w)}
                                >
                                  {mask ? '*'.repeat(w.length) : w}
                                </li>
                              ),
                            )}
                          </ul>
